...
def fundamental_GVD_centered():
    ...
    #criteria_EL_below_3 = dfs["EL on target"] < 2.6
    #criteria_EL_above_2 = dfs["EL on target"] > 2.
    criteria_z_below_1000 = dfs["z um"] > 1000.
    criteria_z_below_2500 = dfs["z um"] <= 2500.

    ROM_criteria = dfs['Nmax'] > 22
    #outGVD0= dfs[GVD0][['day','shot', 'z um', 'EL on target','Nmax','central', 'central2' ]]
    #print(outGVD0)

    overall_shots = dfs[criteria_centered & criteria_non_zero & criteria_PP_out & GVD900 & GVD600 & criteria_z_below_1000 & criteria_z_below_2500][["shot"]]
    print(len(overall_shots), "all shots with HHG content centered")


    shots_ROM = dfs[criteria_centered & ROM_criteria & criteria_PP_out & GVD900 & GVD600 & criteria_z_below_1000 & criteria_z_below_2500][["shot"]]
    print(len(shots_ROM), "only ROM")
    sub_dataset1 = dfs[criteria_centered & criteria_non_zero &criteria_PP_out & GVD900 & GVD600 & criteria_z_below_1000 & criteria_z_below_2500][["Nmax",'central ROM','GVD in fs^2','z um', 'EL on target', "day"]]
    sub_dataset2 = dfs[criteria_centered & criteria_non_zero &criteria_PP_out & GVD900 & GVD600 & criteria_z_below_1000 & criteria_z_below_2500][["Nmax",'central ROM','GVD in fs^2','z um', 'EL on target', "day"]]
    print(sub_dataset1)

    below_1500 = sub_dataset1["z um"] <=1500.

    above_1600 = sub_dataset2[ "z um"] > 1500.





    x = sub_dataset1[above_1600][["Nmax"]]
    print(len(x), " shots ")
    y = sub_dataset1[above_1600][['EL on target']]
    plt.xlabel("Nmax")
    plt.ylabel("energy on target in [J]")
    plt.xlim(10,50)

    plt.figure(1)
    plt.scatter( x, y,alpha=0.1, color = "g", label = "1600-2500 um")
   # plt.colorbar()
    plt.legend()


    x2 = sub_dataset2[below_1500][["Nmax"]]
    print(len(x2), " shots ")
    y = sub_dataset2[below_1500][['EL on target']]
    plt.xlabel("Nmax")
    plt.ylabel("energy on target in [J]")
    ...
    plt.scatter( x2, y,alpha=0.1, color = "r", label = "1000-1500 um")
   # plt.colorbar()
    plt.legend()
    plt.show()
